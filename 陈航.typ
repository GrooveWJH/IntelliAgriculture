...
#abstract-page(
  body: [
    本论文以“自然生态智慧农业大棚控制系统”为主题，设计并实现了一个面向测试与研究的仿真平台。系统基于多级控制架构，结合PID控制、模糊控制和Smith预测控制等常见算法，完成了对温度、湿度、光照强度和二氧化碳浓度等环境参数的动态调节。前端采用React与TypeScript开发，数据管理使用IndexedDB，保障了在浏览器端处理大规模时序数据的效率。针对数据量大、实时性要求高的问题，系统设计了双层存储架构与动态采样策略，以提升数据查询与更新性能。

    初期，仿真模型采用了三角函数生成的简化数据，但随着功能需求的增长，其在环境反馈、参数耦合等方面的不足逐渐暴露。为提升仿真效果，进一步开发了基于实际天气数据驱动的环境模拟系统，将外部气象条件与大棚物理特性相结合，并将控制操作作为输入，实现了各环境参数间符合物理规律的耦合关系，使仿真结果更具参考价值。

    系统还集成了实时监测、智能报警、数据分析与能效评估等功能，初步具备支持农业生产决策的能力。经过测试验证，系统在功能完整性、控制效果和响应速度方面表现良好，具备较高的应用潜力，但在用户体验、移动端适配和智能化决策支持等方面仍有不足，后续将根据实际需求持续优化。

    总体而言，本仿真平台为智慧农业大棚环境控制的研究提供了一种可行的技术方案，通过控制算法与物理模型的结合，实现了较为真实的环境仿真与调控，为现代农业的智能化发展奠定了基础。
  ],
  keywords: (
    "智慧农业大棚",
    "环境控制仿真",
  ...
  body-en: [
    This thesis focuses on the design and implementation of a simulation platform for a natural ecological intelligent greenhouse control system, aimed at testing and research applications. The system is built on a multi-level control architecture, integrating commonly used algorithms such as PID control, fuzzy control, and Smith predictor control to dynamically regulate environmental parameters including temperature, humidity, light intensity, and CO2 concentration. The user interface is developed using React and TypeScript, while IndexedDB is employed for efficient time-series data management on the client side. To address challenges posed by large data volumes and real-time performance demands, a dual-layer storage architecture and dynamic sampling strategy were designed to enhance data retrieval and update efficiency.

    Initially, the simulation model employed simplified trigonometric data generation, which gradually revealed limitations in environmental feedback and parameter coupling as system requirements evolved. To improve simulation realism, a weather-driven environmental simulation system was developed, combining external meteorological factors with greenhouse physical characteristics. Control system operations are used as inputs to establish physically accurate relationships among environmental parameters, significantly enhancing the credibility of simulation results.

    Additionally, the system integrates real-time monitoring, intelligent alarming, data analysis, and energy efficiency assessment functionalities, providing preliminary support for agricultural decision-making. Test results demonstrate satisfactory performance in terms of functional completeness, control accuracy, and response speed, indicating promising application potential. However, limitations remain in user experience, mobile compatibility, and intelligent decision support, which will be addressed in future iterations.

    Overall, this simulation platform offers a practical technical solution for intelligent greenhouse environmental control research. By combining control algorithms with physical modeling, it achieves realistic environmental simulation and regulation, laying a solid foundation for the intelligent development of modern agriculture.
  ],
  keywords-en: (
...
== 研究背景和意义
=== 研究背景
随着人口增长和耕地减少，传统农业难以满足需求。智能农业作为现代农业发展方向，正在改变传统农业生产模式@YanLiang_DianQiZhiNengKongZhiJiShuZhuTuiZhiHuiNongYeFaZhan_2025。智能农业是将信息技术、自动化技术与农业生产结合，实现农业生产精准化、自动化和智能化管理的新型生产方式。
...
中国农业大学开发的"智慧农业云平台"将物联网、云计算和大数据技术整合在一起，实现了对全国各地温室的统一监控和管理。该平台在北京、山东、江苏等地的现代农业园区得到应用，覆盖温室面积超过5000公顷，为农业生产者提供了环境监控和市场预测服务。

浙江大学研究团队开发的"农业物联网与智能决策系统"在杭州、宁波等地的设施农业中得到应用。该系统使用多源异构数据融合技术，结合作物生长模型，实现了灌溉施肥的精准控制。与传统方式相比，该系统节水30%，节肥25%，产量提高15%-20%。
= 系统相关技术介绍
...
=== TypeScript与React框架
TypeScript是JavaScript的超集 @_typescriptXiangMuKaiFaShiZhan_，增加了静态类型系统。在系统开发中，TypeScript的静态类型检查能在编译阶段发现错误，提高代码质量。在温室控制系统这样的应用中，数据结构和接口定义很重要，TypeScript的接口和类型别名功能使数据模型定义更清晰。React框架采用组件化开发方式，使界面结构清晰，便于维护和扩展@QinPeng_JiYureact_2024。系统使用React Hooks管理组件状态，简化了状态管理逻辑，提高了代码可读性。
...
Ant Design支持主题定制，可以根据用户偏好调整界面样式。组件库的TypeScript支持完善，与系统开发环境配合良好，提高了开发效率。系统还使用了Ant Design的布局组件，实现了灵活的页面布局，支持不同设备的显示需求。
...
#figure(
  ...
  supplement: [图],
  image("img/2025-04-03-21-09-24.png", width: 20%),
)
...
= 系统测试与评估
本章对智慧农业大棚控制系统仿真平台进行全面测试与评估，验证系统的功能正确性、性能指标和用户体验。通过系统化的测试方法和客观的评估标准，确保系统能够满足实际应用需求，为系统的后续优化和改进提供依据。
...
在控制器响应测试中，分别测试了PID控制器、模糊控制器和Smith预测控制器的响应特性和控制效果。测试结果表明，PID控制器在线性系统中表现稳定，模糊控制器在非线性系统中适应性较好，Smith预测控制器减少了时延系统的振荡。

在子系统功能测试中，对通风、加湿、补光、灌溉、CO2和遮阳这六个环境控制子系统进行了功能测试。测试结果显示这些子系统在自动模式和手动模式下都能正常工作，功率调节分辨率达到1%，模式切换比较平滑，控制延迟在500毫秒以内。

在控制策略综合测试中，通过模拟不同环境条件和作物生长阶段，测试了系统控制策略的有效性和协同控制能力。测试结果显示系统能应对各种环境条件，协同控制机制减少了子系统间的冲突，节能模式下能耗降低了大约35%。
...
#figure(
  ...
  table(
    ...
    [系统维护], [各类维护操作], [服务影响最小化], table.hline(),
  ),
  caption: [系统长期稳定性测试结果],
)
...
本章通过功能测试、性能测试和用户体验评估，对自然生态智慧农业大棚控制系统仿真平台进行了全面的测试与评估。通过分析测试结果，我对系统的优势与不足进行了客观评价，并分析了系统满足需求的程度。

=== 系统优势分析

自然生态智慧农业大棚控制系统仿真平台在功能完备性方面表现较好。系统实现了智能温室环境监测的核心功能，能够准确采集温度、湿度、光照和二氧化碳浓度等关键环境参数。系统实现了通风、加湿、补光、灌溉、二氧化碳和遮阳六个自动化控制子系统，提供了数据存储、历史查询和趋势分析功能，以及直观的实时监控和控制界面。

在控制精度和实时性方面，系统表现出较好的性能。传感器数据采集精度高，各参数测量偏差都低于行业标准要求。控制响应速度快，平均响应时间约830毫秒。PID控制器在线性系统中的稳态误差低于1%，模糊控制器也显示出良好的适应性。

长期运行测试证明系统具有较高的可靠性。系统连续运行30天没有出现崩溃，资源使用合理，没有明显的内存泄漏问题。在负载波动和极端条件下也能保持稳定，系统维护对服务的影响也很小。

=== 系统不足分析

系统高级功能的易用性有待提高。数据分析功能的用户满意度评分只有3.5分，高级查询界面操作比较复杂，系统配置的层级结构过深，控制策略的编辑方式也不够直观。

系统在移动端的适配方面存在问题。小屏幕上的图表展示效果不好，触摸操作的精度不高，部分高级功能在移动端无法使用，离线工作的能力也比较有限。

系统的扩展性存在局限。设备接入需要手动配置，不同厂商的设备适配存在困难，对大规模场景的支持不足，与第三方系统的集成接口也不够完善。
